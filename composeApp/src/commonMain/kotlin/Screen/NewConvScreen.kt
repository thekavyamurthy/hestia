...
val NewConvScreen by navDestination<Unit> {
    ...
    Column (
        ...
        Row {
            Button(onClick = {
                scope.launch {
                    val conv = APIClient.newConv(otherUser)
                    if (conv.id != 0L) {
                        navController.replace(ChatScreen, ChatParams(ConversationType.DIRECT, conv.id, conv.name))
                    } else {
                        isVisible = true
                    }
                }
